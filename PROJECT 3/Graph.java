...
/**
 * This class reuses and extends the implementation of an
 * undirected graph from lab 10. It implements a generic undirected graph where
 ...
 * 
 * Important note: The code is based on the implementation of an undirected graph
 * and the corresponding shortest path algorithm as found in:
 * Weiss, Mark Allen. "Graphs and Paths." 
 * In Data Structures and Problem Solving Using Java. : Pearson Education Inc., 2010. .
 * 
 * @author Ivan Evtimov
...
public class Graph<K, E>{
   ...
   * @author Ivan Evtimov
   */
  public class UnDirectedGraphNode<K,E> implements Comparable<UnDirectedGraphNode>{
    ...
    /**
     * the equals method compares the node's key with a passed other node
     * and returns true if and only if they have the same key
     * @return true if the keys of the two nodes are identical
     */
    public boolean equals( UnDirectedGraphNode secondNode ){
      return this.key.equals( secondNode.key );
    }
    
    /**
     * This method compares two nodes based on their distance from the starting
     * node (given that Djikstra' BFS algorithm has been called). If the two nodes
     * have the same distance, then the method returns 0. This also applies
     * in the case where Djikstra's BFS hasn't been called, in  which case all nodes will
     * be considered identical. The equals method of this class is used to ensure
     * that this isn't the same node being compared to itself.
     * @param rhs the node we are comparing this one to
     * @return negative number if this node is closer, positive if the parameter node is closer
     */
    public int compareTo( UnDirectedGraphNode rhs ){
      ...
      //if not, get the difference in shortest distance and return it
      return this.shortestDist - rhs.shortestDist;
    }
    
    /**
     * returns shortest distance to start node after BFS.
     * @return the shortest distance from this node to the start node of the bfs
     */
    ...
    /**
     * Returns the value mapped in this node.
     */
    public E getValue(){
      return this.value;    
    }
 ...
 /////////////////////////// Inner Path Class ////////////////////////////////////
  ...
  /**
   * Private method to reset all nodes' shortest distance and 
   * similar stuff. Essential for the operation of djikstra's algorithm.
   */
  ...
  /**
   * Method to do a BFS on the weighted undirected graph
   * using Djikstra's algorithm. It finds the shortest path to all nodes
   * in the graph given a starting node's key
   * @param k1 the key of the starting node for the BFS
   * @throws NoSuchElementException if the node isn't found in the graph
   */
  public void bfs( K k1 ) throws NoSuchElementException{
    ...
    UnDirectedGraphNode startNode = vertCont.get( k1 );
     ...
     //check for exception
    if( startNode == null ){
      throw new NoSuchElementException( "Nothing mapped to k1!" );
    }
    
    resetAll();//reset all - essential to the algorithm
   ...
   * between two nodes and return the path of nodes (their keys) in an array list.
   * @param k1 the key of the first node
   * @param k2 the key of the second node
   * @return the shortest path between nodes with keys k1 and k2 in terms of keys
   * @throws NoSuchElementException if either k1 or k2 is not present in the graph
   */
  public ArrayList<K> findShortestPath( K k1, K k2 ) throws NoSuchElementException{
    ...
    //call the breadth-first search with the k1 as starting node
    bfs( k1 );
    ...
    ArrayList<K> shortestPathReverseArr = new ArrayList<K>();

    //initialize a currNode for the iteartion and set it to endNode's key
    K currNodeKey = k2;
      
    //do the itearation
    while( currNodeKey.equals( k1 ) == false ){
      
        //add the currNode to the path
        shortestPathReverseArr.add( currNodeKey );
        
        //update currNode to point to the previous one (previous was determined by shortest path algorithm)
        currNodeKey = (K) vertCont.get( currNodeKey ).previous.key;
    }
    ...
    ArrayList<K> correctOrderArr = new ArrayList<K>();
  ...
  /**
   * designed to return element mapped in key with given value.
   * @param key the key where the element is mapped
   * @return the element
   */
  public Object get( K key ){
    return  vertCont.get( key ).value;
  }
  
  /**
   * This method is designed to find all nodes within a
   * given radius of a given node (specified by key).
   * @param startKey the key of the node we want to be in the center
   * @param radius the cutoff after which nodes with a longer shortest path are not included 
   * @return arraylist of keys of nodes within the given radius of the startKey node
   */
  public ArrayList<K> radius( K startKey, int radius ){
  
    //call djikstra starting from the center specified
    bfs( startKey );
    
    //get all the keys in this map
 //   Set<K> keySet = vertCont.keySet();
    
    //declare some vars for the loop
    int currDist;
    UnDirectedGraphNode tempNode = null;
    
    //declare the array that will store all the keys in this radius
    ArrayList<K> keysWithinRadiusArr = new ArrayList<K>();
    
    //get all the nodes in this graph in a TreeSet (this will allow access in sorted order)
    TreeSet<UnDirectedGraphNode> nodesSet = new TreeSet<UnDirectedGraphNode>( vertCont.values() );
    
    //iterate over the keys
    for( Iterator it = nodesSet.iterator(); it.hasNext(); ){
    
      //get the node associated with that key
      tempNode = (UnDirectedGraphNode) it.next();
      
      //for each, check corresponding node's shortest distance
      currDist = tempNode.shortestDist;
      
      //add that node's key to the array only if it is within the radius
      if( currDist < radius ){
        keysWithinRadiusArr.add( (K) tempNode.key );
      }
    }
    
    //return that array
    return keysWithinRadiusArr;
    
  }
  
  /**
   * returns shortest distance between two nodes (no path!)
   * @return shortest distance between two nodes (combined sum of edges)
   */
  public int findShortestDistance( K k1, K k2 ){
  
    //call bfs
    bfs( k1 );
    
    //get the second node's shortest distance and return it
    return vertCont.get( k2 ).shortestDist;
